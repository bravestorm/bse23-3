...
choice = ""
# Set a flag to keep track if the menu has been spoken or not
menu_spoken = False
    ...
    while True:
        ...
        if len(choice) == 1:
            if choice == "1":
                ...
                subprocess.Popen(["espeak", "starting obstacle detection in the back"])
                print("starting obstacle detection in the back")
                obstaclebehinddemo.start_obstacle_behind()
                menu()
                break
            elif choice == "6":
                subprocess.Popen(["espeak", "stoping obstacle detection in the back"])
                ...
                subprocess.Popen(["espeak", "shutting down the smart wearable assistant"])
                sys.exit()
                menu()
                break
            else:
...
def menu():
    ...
    if not menu_spoken:
        ...
        time.sleep(3)
        # Set the flag to indicate that the menu has been spoken
        menu_spoken = True
    get_first_choice()
