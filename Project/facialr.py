...
def facial_recognition():
    ...
    # loop over frames from the video file stream
        ...
        for encoding in encodings:
            ...
            subprocess.Popen(["espeak",name]).wait()
            ...
            if True in matches:
                ...
                if currentname != name:
                    currentname = name
                    print(currentname)
                    subprocess.Popen(["espeak", currentname])
...
# Function to start the facial recognition process in a separate thread
def start_facial_recognition():
    # Reset the stop event flag to allow starting the process again
    stop_event.clear()
    # Create a new thread and start the facial recognition function
    facial_recognition_thread = threading.Thread(target=facial_recognition)
    facial_recognition_thread.start()

# Function to stop the facial recognition process
def stop_facial_recognition():
    # Set the stop event to signal the facial recognition process to stop
    stop_event.set()
    # stop the timer and display FPS information
    #fps.stop()
    #print("[INFO] elapsed time: {:.2f}".format(fps.elapsed()))
